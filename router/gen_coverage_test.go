...
// TestCheckUsage_ModuleNameSubPath 验证 -moduleName 为 go.mod 子路径时仍正常执行
func TestCheckUsage_ModuleNameSubPath(t *testing.T) {
    dir := t.TempDir()
    goMod := "module im-go\n\ngo 1.22\n"
    if err := os.WriteFile(filepath.Join(dir, "go.mod"), []byte(goMod), 0644); err != nil {
        t.Fatal(err)
    ...
    orig := *ModuleName
    *ModuleName = "im-go/web"
    t.Cleanup(func() { *ModuleName = orig })

    // 不 panic 即通过, 警告输出到 stdout
    checkUsage(dir, dir, *ModuleName)
}

// TestCheckUsage_NoGoMod 验证无 go.mod 且未指定 moduleName 时不 panic
func TestCheckUsage_NoGoMod(t *testing.T) {
    dir := t.TempDir()
    orig := *ModuleName
    *ModuleName = ""
    t.Cleanup(func() { *ModuleName = orig })
